...
listado2 = leer_csv('./Archivos_Python/Prácticas/data/csv/Práctica10Univ.csv')
...
def proceso_completo(lista_urls):
    dict_sol = {}
    datos = ['seal', 'motto_latin', 'motto_spanish', 'motto_english', 'type', 'established','affilitations', 'chancellor', 'vice_chancellor', 'rector', 'students', 'location', 'campus', 'colors' , 'website', 'logo' ]
    for url in lista_urls:
        try:
            sol = sacar_datos(url[3], datos)
            dict_sol[url[2]] = sol
        except:
            next
    guardar_json(dict_sol, file_path='./Archivos_Python/Prácticas/data/json/Práctica10Fin.json')

proceso_completo(listado2)
